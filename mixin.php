...
define('MIXIN_WOOCOMMERCE_VERSION', '1.2.3');
...
function mixin_init()
{
    ...
    require_once(__DIR__ . '/lib/mixin/init.php');

    class WC_Gateway_Mixin extends WC_Payment_Gateway
    {
        public function __construct()
        {
            error_log("WC_Gateway_Mixin __construct");
            global $woocommerce;
        ...
        {
            error_log("mixin admin_options");
            ?>
            <h3><?php _e('MiXin', 'woothemes'); ?></h3>
            <p><?php _e('Accept Bitcoin through the mixin.one and receive payments in euros and US dollars.<br>
        ...
        {
            error_log("mixin init_form_fields");
            $this->form_fields = array(
                'enabled' => array(
                    'title' => __('Enable MiXin', 'woocommerce'),
                    'label' => __('Enable Cryptocurrency payments via MiXin', 'woocommerce'),
                    'type' => 'checkbox',
                    'description' => '',
                    'default' => 'no',
                ),
                ...
                'title' => array(
                    ...
                    'default' => __('Cryptocurrencies via MiXin (more than 50 supported)', 'woocommerce'),
                ),
                ...
                'test' => array(
                    ...
                    'default' => 'no',
                    'description' => __('To test on <a href="https://sandbox.mixin.one" target="_blank">MiXin Sandbox</a>, turn Test Mode "On".
                    Please note, for Test Mode you must create a separate account on <a href="https://sandbox.mixin.one" target="_blank">sandbox.coingate.com</a> and generate API credentials there.
        ...
        {
            error_log("mixin process_payment");
            ...
            $token = get_post_meta($order->get_id(), 'mixin_order_token', true);

            if (empty($token)) {
                $token = substr(md5(rand()), 0, 32);

                update_post_meta($order_id, 'mixin_order_token', $token);
            }
            ...
            $order = \CoinGate\Merchant\Order::create(array(
                ...
                'callback_url'      => trailingslashit(get_bloginfo('wpurl')) . '?wc-api=wc_gateway_mixin',
        ...
        {
            error_log("mixin payment_callback");
            $request = $_REQUEST;
            ...
            try {
                ...
                if (!$cgOrder) {
                    throw new Exception('MiXin Order #' . $order->get_id() . ' does not exists');
                }
                ...
                switch ($cgOrder->status) {
                    ...
                    case 'invalid':
                        ...
                        break;
                    case 'expired':
                        if($order->get_payment_method() === "mixin") {
                            $order->update_status($wcOrderStatus);
                    ...
                    case 'canceled':
                        $order->update_status($wcOrderStatus);
                        $order->add_order_note(__('Buyer canceled the invoice.', 'mixin'));
                        break;
                    case 'refunded':
                        $order->update_status($wcOrderStatus);
                        $order->add_order_note(__('Payment was refunded to the buyer.', 'mixin'));
                        break;
                }
            } catch (Exception $e) {
        ...
        {
            error_log("generate_order_statuses_html");
            ob_start();
            ...
            <tr valign="top">
                ...
                <td class="forminp" id="mixin_order_statuses">
                    <table cellspacing="0">
                        <?php
        ...
        {
            error_log("validate_order_statuses_field");
            $orderStatuses = $this->get_option('order_statuses');
        ...
        {
            error_log("save_order_statuses");
            $cgOrderStatuses = $this->cgOrderStatuses();
            $wcStatuses = wc_get_order_statuses();

            if (isset($_POST['woocommerce_mixin_order_statuses']) === true) {
                $cgSettings = get_option('woocommerce_mixin_settings');
                $orderStatuses = $cgSettings['order_statuses'];

                foreach ($cgOrderStatuses as $cgStatusName => $cgStatusTitle) {
                    if (isset($_POST['woocommerce_mixin_order_statuses'][$cgStatusName]) === false)
                        continue;

                    $wcStatusName = $_POST['woocommerce_mixin_order_statuses'][$cgStatusName];
                ...
                update_option('woocommerce_mixin_settings', $cgSettings);
            }
        }
